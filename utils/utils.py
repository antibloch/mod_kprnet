...
def spherical_projection(points,
    ...
    cl_map_2d = np.zeros((H, W), dtype=np.int32)
    ...
    cl_maps = []
    unique_clsses = np.unique(labels_ord)
    print(f"Unique classes in reference labels: {unique_clsses}")
    colors_labels = np.zeros((len(labels_ord), 3), dtype=np.float32)
    for cl in range(len(unique_clsses)):
        mask = (labels_ord == unique_clsses[cl])
        mask = np.squeeze(mask, axis = 1)
        idx = indices_ord[mask]

        random_color =np.random.rand(3)
        colors_labels[mask, :] = random_color
