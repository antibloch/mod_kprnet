...
def check():
    ...
    depth_img_mean = 0.0
    depth_img_square_mean = 0.0

    refl_img_mean = 0.0
    refl_img_square_mean = 0.0
    ...
    for step, items in enumerate(train_loader):
        ...
        depth_img_mean += torch.sum(depth_image, dim =0)
        depth_img_square_mean += torch.sum(depth_image ** 2, dim = 0)

        refl_img_mean += torch.sum(reflectivity_image, dim = 0)
        refl_img_square_mean += torch.sum(reflectivity_image ** 2, dim = 0)

        count += depth_image.shape[0]


    depth_img_mean /= count
    depth_img_square_mean /= count

    refl_img_mean /= count
    refl_img_square_mean /= count

    depth_img_mean = depth_img_mean.cpu().numpy()
    depth_img_square_mean = depth_img_square_mean.cpu().numpy()

    refl_img_mean = refl_img_mean.cpu().numpy()
    refl_img_square_mean = refl_img_square_mean.cpu().numpy()

    D_mean = depth_img_mean
    D_std = (depth_img_square_mean - depth_img_mean ** 2) ** 0.5

    R_mean = refl_img_mean
    R_std = (refl_img_square_mean - refl_img_mean ** 2) ** 0.5

    np.save("D_mean.npy", D_mean)
    np.save("D_std.npy", D_std)
    np.save("R_mean.npy", R_mean)
    np.save("R_std.npy", R_std)
